...
body <- dashboardBody(
  tabItems(
    ...
    tabItem(tabName = "Importation",
            ...
            tabsetPanel(
              tabPanel("Overview", value = "Overview",
                       conditionalPanel("input.data_type == ''",
                                        br(),
                                        br(),
                                        br(),
                                        br(),
                                        fluidRow(
                                          column(1, icon("arrow-left", "fa-4x")
                                          ),
                                          column(2, h2( "Importez les Data par Ici"))
                                        )),
              ...
              tabPanel("Processing", value = "Processing",
                       ...
                       column(3,icon("2"),
                              ...
                              br(),
                              conditionalPanel("input.tran == 'Renaming'",
                                               actionButton("rename_but", "Rename"))),
                       column(3,icon("3"),
                              ...
                              conditionalPanel("input.tran == 'Conversion'",
                                               actionButton("convert_but", "Convert"))
    ...
    tabItem(tabName = "AED",
            ...
            conditionalPanel("input.graph_id == '' ", 
            ...
            conditionalPanel("input.graph_id == 'Analyse Bi-Varie'",
                             fluidRow(
                               column(6,
                                      fluidRow(
                                        column(3,
                                               varSelectInput("bi_var11", label = "Choissir la variables a expliquer", data = NULL, selected = NULL)),
                                               #selectInput("bi_var11", "Choisir une Variable", choices = c("Choose"="",NULL))),
                                                  
                                        column(3,
                                               varSelectInput("bi_var12", label = "Choissir la variables a expliquer", data = NULL, selected = NULL)
    ...
    tabItem(tabName = "APD",
        ...
        #    h4("L'analyse predictive est un type d'analyse de donnees qui utilise les statistiques, la Data Science, le machine learning et d'autres techniques pour predire ce qui va se passer dans le futur. "),
         #   h4("Elle consiste a repondre a la question : qu'est-ce qui a le plus de chances de se produire a l'avenir sur la base des tendances historiques ?"),
            h4("Dans cette partie , Vous allez commencez par :"),
            h5("(1) chiossir les variables a expliquer et explicatives"),
            h5("(2) Diviser les Donnees en Train et Test par le bouton 1"),
            h5("(3) Choissir les Steps que vous voulez inclure dans la recette des transformations a appliquer sur l'ensemble d'entrainement et appuyer sur le bouton 2 (Lissez bien les description et le type de variables que vous devez choisir pour chaque Step)"),
            h5("(Conseil) Lisez le resume de recette qui s'affiche ci-dessous pour mieux comprendre les resultats des transformations"),
            h5("(Bonus) C'est possible de revenir a l'onglet AED pour visualiser les donnees transformees"),
            h5("(4) Entrainez les modele de classification (Arbre de Decision, KNN, Foret Aleatoire et Regression Logestique) avec 5-fold CV en appuyant sur le boutton 3 "),
            h5("(5) Bravo ! Maintenant Passez a l'onglet d'evaluation pour comparer les performances des modeles"),
            h5("(6) Passer a l'onglet de Final Predict pour appliquer le mauilleur modele sur l'ensemble de Test"),
            br(),
            br(),
            tabsetPanel(
                                        ...
                                        #  column(3,
                                         #        checkboxInput("id_smote", "Add a step smote?"),htmlOutput("Sd_smote"),
                                          #       varSelectInput("id_smote_var", "Choisir les variables categorielles avec les classes desequilibres",
                                           #                     data = NULL, selected = NULL, multiple = FALSE)),
                                          column(3,
                                                 checkboxInput("id_down", "Add a step downsample?"),htmlOutput("Sd_down"),
                                                 varSelectInput("id_down_var", "Choisir la variable categorielle avec les classes desequilibres", 
                                                                data = NULL, selected = NULL, multiple = FALSE)),
                                        column(3,
                                               checkboxInput("id_up", "Add a step upsample?"),htmlOutput("Sd_up"),
                                               varSelectInput("id_up_var", "Choisir la variable categorielle avec les classes desequilibres", 
                                                              data = NULL, selected = NULL, multiple = FALSE)),
                                        ),
                                          fluidRow(
              ...
              tabPanel("Evaluation" , value = "eval",
                       fluidRow(
                         column(6,DT::dataTableOutput("res_tab1")),
                         column(6,DT::dataTableOutput("m_tab1"))
                       ),
                       fluidRow(
                         column(6,DT::dataTableOutput("res_tab2")),
                         column(6,DT::dataTableOutput("m_tab2"))
                       ),
                       
                       fluidRow(
                         column(6,DT::dataTableOutput("res_tab3")),
                         column(6,DT::dataTableOutput("m_tab3"))
                         
                       ),
                       fluidRow(
                         column(6,DT::dataTableOutput("res_tab4")),
...
server <- function(input, output, session){
  ...
  observeEvent(input$convert_but, {
  ...
  observeEvent(c(input$id_data,data0$df),{
    if (is.null(data_prep$df)){
      ...
      updateVarSelectInput(session, "bi_var11", data = data0$df, selected = NULL)
      
      updateVarSelectInput(session, "bi_var12", data = data0$df, selected = NULL)
...
#    updateVarSelectInput(session, "id_smote_var", data = data0$df, selected = NULL)
    updateVarSelectInput(session, "id_median_var", data = data0$df)
    updateVarSelectInput(session, "id_mode_var", data = data0$df)
    updateVarSelectInput(session, "id_up_var", data = data0$df, selected = NULL)
    updateVarSelectInput(session, "id_down_var", data = data0$df, selected = NULL)
...
#    )
 # })
  ...
  observeEvent(input$res_but,{
    ...
    output$res_tab1 <-DT::renderDataTable({
      NULL
    })
    output$m_tab1 <-DT::renderDataTable({
      NULL
    })
    output$res_tab2 <-DT::renderDataTable({
      NULL
    })
    ...
    output$res_tab4 <-DT::renderDataTable({
      NULL
    })
...
#    updateCheckboxInput(session, "id_smote", value = FALSE)
 ...
 #     l1 <- length(input$id_smote)
  #    for (x in 1:l1) {
        data1$rec <- data1$rec%>%
          themis::step_smote(!!input$id_smote_var, skip = TRUE)
#      }
    }
  })
 ...
 #     l2 <- length(input$id_down)
  #    for (i in 1:l2) {
        data1$rec <- data1$rec%>%
          themis::step_downsample(!!input$id_down_var)
                                  #[[i]])
#      }
    }
  })
  ...
  observeEvent(input$id_up,{
    ...
    if((input$id_up == TRUE) & (!is.null(input$id_up))){
        data1$rec <- data1$rec%>%
          themis::step_upsample(!!input$id_up_var)
...
#  observeEvent(input$id_unknown,{
 #   if(input$id_unknown == TRUE){
  ...
  observeEvent(input$rec_but,{
    ...
    output$tableee <- DT::renderDataTable({
      DT::datatable(juice(data_prep$df),
                    options = list(
                      scrollX = TRUE
                    ))
...
#  glm_spec <- logistic_reg()%>%
 #   set_engine("glm")
  ...
  observeEvent(input$pred_but,{
    ...
    folds <- vfold_cv(data_train$df, v = 2)
 ...
 #     if (c == 2)  {
  ...
  #    }
   ...
   # })
    ...
    output$res_tab3 <-  DT::renderDataTable({
